...
def testGenEmbedds():
	...
	parser.add_argument("-m", "--model", type=str, default="models/13b/ggml-model.bin")
...
def loopThrgandGetEmbedd(loc_json):
	...
	# add trained model
	llm = Llama(model_path="models_lamma/codellama-7b.Q6_K.gguf", embedding=True)
    ...
    for eachJSONL in getlistofjsonl:
        ...
        with jsonlines.open(loc_json+'/'+eachJSONL, 'r') as json_file:
            for eachLine in json_file:
                dictHere[eachLine["docID"]] = llm.create_embedding(eachLine["reviews"])
                # print(embeddings)
        np.save(eachJSONL[:-6]+".npy", dictHere)
        sys.exit(0)


loopThrgandGetEmbedd("C:/Users/asa/Desktop/Projects/22Math_recSys/data/Final_All/allWithComplexity")