...
INSTRUCTIONS = {"qa": "Retrieve semantically similar text."}
...
def createIndex(main_data_, model_):
    ...
    for start_idx in range(0, len(main_data_), batch_size):
        ...
        embeddings_batch = model_.encode([instruction+tit for tit in titles_batch], convert_to_numpy=True, device='cuda')
        faiss.normalize_L2(embeddings_batch)
        index.add(embeddings_batch.astype('float32'))
    faiss.write_index(index, "data/base_/abstracts/abs_llmebd.index")

def main_faiss():
    instruction = INSTRUCTIONS["qa"]
    model = SentenceTransformer("BAAI/llm-embedder", device='cuda', trust_remote_code=True)
    ...
    index = faiss.read_index("data/base_/abstracts/abs_llmebd.index")
    batch_size = 5000
    for batch_start in range(0, len(test_['document_id']), batch_size):
        ...
        embeddings_batch = model.encode([instruction+tit for tit in batch_titles], convert_to_numpy=True, device='cuda')
        faiss.normalize_L2(embeddings_batch)
