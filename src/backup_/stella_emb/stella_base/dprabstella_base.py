...
def createIndex(main_data_,used_data,msc='',model_name_or_path='base'):
    ...
    tokenizer = AutoTokenizer.from_pretrained("approach0/dpr-cocomae-520")
    model = AutoModel.from_pretrained("approach0/dpr-cocomae-520")
    ...
    #for start_idx in range(0, 10, batch_size):
        ...
        #sys.exit(0)
        encoded_inputs = tokenizer(titles_batch, padding=True, truncation=True, return_tensors="pt")
...
def main_faiss(main_data, used_data, cross_features=False, subset='test', msc='', model_name_or_path='base'):
    ...
    for batch_start in range(0, len(subset_df['document_id']), batch_size):
        ...
        encoded_inputs = tokenizer(batch_titles, padding=True, truncation=True, return_tensors="pt")
        #embeddings_batch = model.encode(titles_batch, convert_to_numpy=True, device='cuda')
        #faiss.normalize_L2(embeddings_batch)
        encoded_inputs = {key: value.to(device) for key, value in encoded_inputs.items()}
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    model_dir = 'approach0/dpr-cocomae-520'
    ...
    createIndex(main_data_,['text'],model_name_or_path=model_dir)
    #main_faiss(main_data_,['title','text'],msc='14',subset='test',model_name_or_path=model_dir)
    main_faiss(main_data_,['text'],subset='test',model_name_or_path=model_dir)
