...
def main_faiss():
    ...
    with open("example_recmnds.csv", mode='w', newline='') as file:
        ...
        doc_ids = [7835485,'7660728','5365216','4182403','3995949', '6776518', '2128932', '7317612', '2213649', '958030', '5535377', '7212179', '1462454', '6096092', '5871009', '5076716', '6252000', '1213980', '3903426', '7438400']
        for batch_start in range(0, len(test_['document_id']), batch_size):
            ...
            filtered_df = main_data[main_data['document_id'].isin(doc_ids)]
            filtered_df = np.array(filtered_df)
            print("default ones: ", batch_titles)
            print(type(batch_titles))
            print("My titles: ", filtered_df['text'].tolist())
            sys.exit(0)
            embeddings_batch = model.encode(batch_titles, convert_to_numpy=True, device='cuda')
            faiss.normalize_L2(embeddings_batch)
            _, ranked_indices = index.search(embeddings_batch, 10)
