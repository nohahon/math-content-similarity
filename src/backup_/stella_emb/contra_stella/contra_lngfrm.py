...
output_dir = "./trained_model_ß24_ls"
push_to_hub = True
new_model_name = "contrastive-stella-embeddings"
checkpoint_dir = "./chckpnts_stle_rand_ß24_ls"
...
log_acc_ls = {
    'acc_stp': defaultdict(lambda: 0),
    'lss_stp': defaultdict(lambda: 0),
    'acc_epoch': defaultdict(lambda: 0),
    'lss_epoch': defaultdict(lambda: 0),
}
...
for epoch in range(2):  # Number of epochs
    ...
    for index, row in train_df.iterrows():
        ...
        if len(pos_combs) > 0:
            ...
            log_acc_ls['acc_stp'][index] = accuracy
            log_acc_ls['lss_stp'][index] = epoch_loss
    ...
    accuracy = correct_predictions / total_predictions
    log_acc_ls['acc_stp'][index] = accuracy
    log_acc_ls['lss_stp'][index] = epoch_loss/index
    print(f"Epoch {epoch + 1}: Loss = {epoch_loss / index}, Accuracy = {accuracy * 100:.2f}%")

log_acc_ls = {key: dict(value) for key, value in log_acc_ls.items()}
with open('model_vals.pkl', 'wb') as f:
    pickle.dump(log_acc_ls, f)
