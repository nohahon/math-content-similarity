...
#getGPUproperties()
task_name_to_instruct = {"example": "Given a reseearch paper title, retrieve most similar titles that are related",}
query_prefix = "Instruct: "+task_name_to_instruct["example"]+"\nQuery: "
queries = ['Quantum cohomology of rutherfors operatirs']
model = AutoModel.from_pretrained('nvidia/NV-Embed-v2', trust_remote_code=True).eval()
max_length = 32768
...
def createIndex(main_data_):
    ...
    for start_idx in range(0, len(main_data_), batch_size_):
        ...
        query_embeddings = model.encode(titles_batch, instruction="", max_length=max_length)
        query_embeddings = query_embeddings.cpu().numpy()
        faiss.normalize_L2(query_embeddings)
        index.add(query_embeddings.astype('float32'))
    # Save the index to a file
    faiss.write_index(index, "data/base_/abstracts/abs_nvmembv2_basemebd.index")
...
def main_faiss():
    ...
    index = faiss.read_index("data/base_/abstracts/abs_nvmembv2_basemebd.index")
    batch_size = 100
    for batch_start in range(0, len(test_['document_id']), batch_size):
        ...
        embeddings_batch = model.encode(batch_titles, instruction=query_prefix, max_length=max_length).eval()
        ...
        #print(results)
        with open(f'data/base_/abstracts/scores_/abs_base_{batch_start}.json', 'w') as json_file:
