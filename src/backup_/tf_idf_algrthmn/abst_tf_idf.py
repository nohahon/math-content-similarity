...
def main_tfidf():
    ...
    main_data = zbCitData_st.getMainData()  # Load main data containing 'document_id', 'title', 'text'
...
def createIndex(main_data_):
    ...
    pickle.dump(vectorizer, open('data/abstr_tfidf_vectorizer.pkl', 'wb'))
    tfidf_matrix = vectorizer.fit_transform(main_data_['text'])
    ...
    faiss.write_index(index, "data/abstr_faiss_tfidf.index")
...
def main_faiss():
    ...
    # Loop through each document_id in the test_ dataframe
    index = faiss.read_index("data/abstr_faiss_tfidf.index")
    vectorizer = pickle.load(open('data/abstr_tfidf_vectorizer.pkl', 'rb'))
    vectorizer.fit(main_data['text'])
    batch_size = 5000
    for batch_start in range(0, len(main_data['document_id']), batch_size):
        results = {}
        batch_doc_ids = main_data['document_id'][batch_start:batch_start + batch_size]
        ...
        _, ranked_indices = index.search(query_vectors, 10)
        ...
        results = {int(doc_id): [int(idx) for idx in ranked_doc_ids] for doc_id, ranked_doc_ids in results.items()}
        #print(results)
        with open(f'data/base_/mrdi/tfidf_base_abstr_{batch_start}.json', 'w') as json_file:
            json.dump(results, json_file)
        #sys.exit(0)
