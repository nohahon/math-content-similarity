...
def train_IDs():
    ...
    negIds_t = [pair[1] for pair in trainsamp[1]]
    negIDs_v = [pair[1] for pair in validsamp[1]]
    #print("exclude data: ", set(negIds_t), set(negIDs_v))
    return set(negIds_t+negIDs_v)

def get_data_test(dataset, embed_dir, exclude_Ids):
    """
    Used for testing with millions samples
    So you have two options
    1. Save million pairs as pickle (not efficient)
...
if __name__ == '__main__':
    ...
    #stat_dir = os.path.join(processed_dir, 'data.stat') dont need to print stat
    pt_dir = os.path.join(processed_dir, 'seedToembed.pkl')
    ...
    train_file = os.path.join(processed_dir, 'train_posandnegPairs.pkl')
    #data = load_file(os.path.join(processed_dir, 'data.valid'))
    val_file = os.path.join(processed_dir, 'valid_posandnegPairs.pkl')
    #data = load_file(os.path.join(processed_dir, 'data.test'))
    test_file = os.path.join(processed_dir, 'test_posandnegPairs.pkl')

    if parse.model_type == 'DNN':
        #train(train_file, val_file, test_file, parse.eb_dim,parse.lr,
        ...
        train_svm(train_file, val_file, test_file, pt_dir, processed_dir)
    else:
        print('No Such Model')
