...
def cleanDPRscores(prev_dprScores):
    ...
    for eachD in prev_dprScores.keys():
        ...
        for eachK in prev_dprScores[eachD].keys():
            localDict[re.findall(r"'(.*?)'", eachK)[0]] = prev_dprScores[eachD][eachK]
...
def completeScores():
    """
    We make sure that the list is complete by adding the missing IDS from A0 and DPR coth
    """
    a0scores = extractScores("/beegfs/schubotz/ankit/data/mabowdor/recsys/pya0")
    dprscores = cleanDPRscores(extractScores("/beegfs/schubotz/ankit/data/mabowdor/recsys/dpr"))
    aos = dict()
    for eachD in dprscores.keys():
        absDocids = list(set(dprscores[eachD].keys())-set(a0scores[eachD].keys()))
    ...
    for eachD in a0scores.keys():
        absDocids = list(set(a0scores[eachD].keys())- set(dprscores[eachD].keys()))
...
def combinedScores():
    ...
    with jsonlines.open('rslts_mabowDor_dpr.jsonl', mode='w') as writer:
        ...
        for ea in combinedScores.keys():
            baselinercmnds = dict()
            sortedH = sorted(combinedScores[ea].items(), key=lambda x: x[1], reverse=True)[:11]
