...
def getSimilarities():
    allAbstrs = read_and_print_csv_rows('/beegfs/schubotz/noah/arxMLiv/zbmath_abstracts.csv')
    print(len(allAbstrs))
    alScores = dict()
    ...
    for each_ in alScores.keys():
        sorted_dict[each_] = sorted(alScores[each_].items(), key=lambda x: x[1], reverse=True)
