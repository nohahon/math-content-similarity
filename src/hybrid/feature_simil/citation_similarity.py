...
def getScores():
    ...
    for eachK in references_toseeds.keys():
        if eachK in getDocref.keys():
            ...
            keys = [instruction["key"] + key[0] for key in references_toseeds[eachK]]
            ...
            with open("data_ne/referencesScores/citedIn/"+str(eachK)+'.pkl', 'wb') as f:
...
def arrangeScoresAsRec():
    ...
    for eachK in references_toseeds.keys():
        if eachK in getDocref.keys():
            refToSeeds[zbl_to_ids[eachK]] = [key[1] for key in references_toseeds[eachK]]
    for eachK in seed_references.keys():
        try:
            if eachK in getDocref.keys():
                seedToRef[zbl_to_ids[eachK]] = [key[1] for key in seed_references[eachK]]
        except:
            continue
...
def createResults():
    ...
    for eachSeed in mergeddict.keys():
        sorted_ = sorted(mergeddict[eachSeed].items(), key=lambda x: x[1], reverse=True)
    ...
    with jsonlines.open('rslts_directRefrences.jsonl', mode='w') as writer:
        for eachSeed in rankedS.keys():
            ...
            for id_,pot_rcmnds in enumerate(rankedS[eachSeed][:11]):
                baselinercmnds[str(id_)] = [int(pot_rcmnds[0]), pot_rcmnds[1].item()]
...
def getTrueSeedsHavCit():
    ...
    print("documents whose ref are available: ",len(set(list(idToZBLcit_re.keys())).union(set(list(idToRefrences_o.keys()))).intersection(set(seedIds))))
    ...
    final_links = dict()
    for each_k in set(list(idToRefrences_o.keys())).union(set(list(idToZBLcit_re.keys()))):
        set_o = set()
        set_re = set()
    ...
    #sys.exit(0)
    print("Related in the final: ", set(list(final_links.keys())).intersection(set(seedIds)))
    ...
    print(len(set(list(idToZBLcit_o.keys())).union(set(list(idToZBLcit_re.keys()))).intersection(set(seedIds))))
