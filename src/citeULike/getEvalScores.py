...
def genEmbeddingsBatch():
    alldocs = createDocIdAbPairs("/beegfs/schubotz/ankit/data/citeULike/citeulike-a/raw-data.csv")
    instruction = INSTRUCTIONS["qa"]
    path_ = "/beegfs/schubotz/ankit/code/citeULike/data/"
    tokenizer = AutoTokenizer.from_pretrained('BAAI/llm-embedder')
    model = AutoModel.from_pretrained('BAAI/llm-embedder')
    for i_ in range(0, len(alldocs),1000):
        queries = [instruction["query"] + alldocs[query] for query in list(alldocs.keys())[i_:i_+1000]]
...
def calculateSimilarity(dir_storeddata):
    alldocs = createDocIdAbPairs("/beegfs/schubotz/ankit/data/citeULike/citeulike-a/raw-data.csv")
    getAlldr = os.listdir(dir_storeddata)
...
def getUserPairs(filename):
    ...
    for each_ in dictScores.keys():
        sorted_dict[each_] = sorted(dictScores[each_].items(), key=lambda x: x[1], reverse=True)
    with open('citeUlik_title.pkl', 'wb') as f:
        pickle.dump(sorted_dict,f)
    return seed_and_rcmnds

def getTopScores():
    seedandrsmnds = getUserPairs("/beegfs/schubotz/ankit/data/citeULike/citeulike-a/users.dat")
    ...
    with jsonlines.open('rslts_cituLike.jsonl', mode='w') as writer:
        for eachSeed in scores.keys():
            ...
            resultsdict["idealRcmnds"] = [str(ele) for ele in seedandrsmnds[eachSeed]]
